...
contract MockIPGraph {
    ...
    function getParentIps(
        address ipId
    ) external view returns (address[] memory) {
        return parentIps[ipId].values();
    }

    function getParentIpsExt(
        address ipId
    ) external view returns (address[] memory) {
        return parentIps[ipId].values();
    }
    ...
    function getParentIpsCountExt(
        address ipId
    ) external view returns (uint256) {
        return parentIps[ipId].length();
    }

    function hasAncestorIp(
        address ipId,
        address ancestor
    ...
    function _setRoyalty(
        ...
        if (policyKind == POLICY_KIND_LAP)
            royaltiesLap[ipId][parentIpId] = royaltyPercentage;
        if (policyKind == POLICY_KIND_LRP)
            royaltiesLrp[ipId][parentIpId] = royaltyPercentage;
    }
