...
interface ValidatorNavMobileProps {
  user: User | undefined;
  ...
  return (
    <div className="px-4 py-4 bg-black">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-white font-medium">{user?.name ?? 'Validator Name'}</h3>
          <p className="text-sm text-gray-400">{user?.email ?? 'validator@example.com'}</p>
        </div>
      </div>
    </div>
  );
...
// Main ValidatorNavMobile Component
const ValidatorNavMobile: React.FC<ValidatorNavMobileProps> = ({
  user,
  isMobileMenuOpen,
  setIsMobileMenuOpen,
  handleSignOut,
  ...
  return (
    <nav className="lg:hidden bg-black backdrop-blur-sm sticky top-0 z-10 h-[64px] after:content-[''] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[1px] after:bg-gradient-to-r after:from-white/0 after:via-white/50 after:to-white/0">
      ...
      <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
        <SheetTitle className="hidden">Validator Nav Mobile Menu</SheetTitle>
        <SheetContent
          showCloseButton={false}
          side="right"
          className="w-full p-0 border-r-0 h-full bg-black mt-[64px]"
        >
          {/* Validator Profile Section */}
...
export default ValidatorNavMobile;
