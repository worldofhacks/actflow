...
export const SellerDiscoverAgents = async ({
  filters,
  topics,
  featuredAgents,
  allAgents,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="">
        {/* Header Section */}
        <div className="mb-6 lg:mb-8 flex justify-between items-center md:flex-row flex-col gap-4">
          <Link href="/agent/add">
            <Button className="group flex items-center gap-x-2">
              <PlusCircle className="group-hover:rotate-90 duration-300 transition-transform " />{' '}
              Create agent
            </Button>
          </Link>
        </div>
        ...
        <div className="mb-8 lg:mb-12">
          ...
          <div className="relative">
            <div className="overflow-x-scroll overflow-y-visible scrollbar-hide -mx-4 px-4 lg:mx-0 lg:px-0">
              <div className="flex space-x-4 lg:space-x-6 pb-4" style={{ minWidth: 'max-content' }}>
                {featuredAgents?.map((agent, index) => (
                  <AgentCard key={agent.agentId} index={index} agent={agent} />
                ))}
              </div>
            </div>
          </div>
